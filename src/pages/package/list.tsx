...
// routes
import { PATH_DASHBOARD, PATH_PACKAGE } from '../../routes/paths';
// @types
import { IPackage, PackageStatus } from '../../@types/package';
// _mock_
...
const TABLE_HEAD = [
  { id: 'title', label: 'Package Name', align: 'left' },
...
export default function packageListPage() {
  ...
  const { getMany, remove: deletePackage } = usePackage();
  ...
  async function getVenodrs(reload?: any) {
    ...
    await _result;
    if (_result?.data) {
      enqueueSnackbar(reload ? 'Packages reloaded Successfully' : ' Package Loaded successfully!');
      setTableData(_result?.data);
    } else {
      enqueueSnackbar(' Package Failed to Load success!', {
        variant: 'error',
      });
      setTableData([]);
    }
  ...
  return (
    <>
      <Head>
        <title>Package: List | Minimal UI</title>
      </Head>
