...
import { ITrip, ITripCreateInput, ITripEdit, TripStatus } from 'src/@types/trip';
import { useTrip } from 'src/modules/trip/hooks/useTrip';
import { useUploader } from 'src/modules/cdn/useUploader';
// ----------------------------------------------------------------------

interface FormValuesProps extends Omit<ITripEdit, 'avatarUrl'> {
...
export default function TripEditForm({ isEdit = false, trip }: Props) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      contactMobile: trip?.contactMobile || '',
      contactEmail: trip?.contactEmail || '',
      addressLine1: trip?.address?.addressLine1 || '',
      addressLine2: trip?.address?.addressLine2 || '',
      area: trip?.address?.area || '',
      landmark: trip?.address?.landmark || '',
      city: trip?.address?.city || '',
      pincode: trip?.address?.pincode || '0',
      state: trip?.address?.state || '',
      profileImage:
        (trip?.profileImage && { file: cdnPath(trip?.profileImage), isNew: false }) ||
        undefined,
      country: trip?.country || 'India',
      pan: trip?.pan || '',
      panDoc: (trip?.panDoc && { file: cdnPath(trip?.panDoc), isNew: false }) || undefined,
      gst: trip?.gst || '',
      gstDoc: (trip?.gstDoc && { file: cdnPath(trip?.gstDoc), isNew: false }) || undefined,
      estbId: trip?.estbId || '',
      estbtDoc:
        (trip?.estbtDoc && { file: cdnPath(trip?.estbtDoc), isNew: false }) ||
        undefined,
      cin: trip?.cin || '',
      cinDoc: (trip?.cinDoc && { file: cdnPath(trip?.cinDoc), isNew: false }) || undefined,
      isVerified: trip?.isVerified || false,
  ...
  const values = watch();
  useEffect(() => {
    console.log("trip ", trip?.isVerified)
  ...
  const onSubmit = async (data: FormValuesProps) => {
    ...
    if (data?.profileImage?.isNew === true) {
      ...
      if (_fileUploaded?.data?.filename) {
        _tripUpdateParams.profileImage = _fileUploaded?.data?.filename;
      }
    }
    if (data?.panDoc?.isNew === true) {
      ...
      await _fileUploaded;
      if (_fileUploaded?.data?.filename) {
        _tripUpdateParams.panDoc = _fileUploaded?.data?.filename;
      }
    }
    if (data?.gstDoc?.isNew === true) {
      ...
      if (_fileUploaded?.data?.filename) {
        _tripUpdateParams.gstDoc = _fileUploaded?.data?.filename;
      }
    }
    if (data?.estbtDoc?.isNew === true) {
      ...
      if (_fileUploaded?.data?.filename) {
        _tripUpdateParams.estbtDoc = _fileUploaded?.data?.filename;
      }
    }
    if (data?.cinDoc?.isNew === true) {
      ...
      if (_fileUploaded?.data?.filename) {
        _tripUpdateParams.cinDoc = _fileUploaded?.data?.filename;
      }
    }
    ...
    try {
      const _updatedItem = await update(trip?.id, _trip);
      await _updatedItem;
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={8}>
          <Card sx={{ p: 3, m: 2 }}>
            ...
            >
              <RHFTextField name="title" label="trip Title *" />
          ...
          <Card sx={{ p: 3, m: 2 }}>
            ...
            >
              ...
              >
                <RHFTextField name="gst" label="trip's GST" />
                <RHFUploadAvatar
                  name="gstDoc"
              ...
              >
                <RHFTextField name="pan" label="trip's PAN" />
                <RHFUploadAvatar
                  name="panDoc"
              ...
              >
                <RHFTextField name="cin" label="trip's CIN" />
                <RHFUploadAvatar
                  name="cinDoc"
              ...
              >
                <RHFTextField name="estbId" label="trip's Establishment  Doc" />
                <RHFUploadAvatar
                  name="estbtDoc"
        ...
        <Grid item xs={12} md={4}>
          ...
          <Card sx={{ mt: 3, pt: 2, pb: 2, px: 3 }}>
            ...
            >
              {' '}
              <Typography> trip Credentials</Typography>
