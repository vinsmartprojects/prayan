...
// routes
import { PATH_DASHBOARD, PATH_CLIENT } from '../../routes/paths';
// @types
...
// sections
import { ClientTableToolbar, ClientTableRow } from '../../sections/client/list/index';
...
const TABLE_HEAD = [
  { id: 'title', label: 'Client Name', align: 'left' },
...
export default function clientListPage() {
  ...
  const { getMany, remove: deleteClient } = useClient();
  ...
  async function getVenodrs(reload?: any) {
    ...
    await _result;
    if (_result?.data) {
      enqueueSnackbar(reload ? 'Client reloaded Successfully' : ' Client Loaded successfully!');
      setTableData(_result?.data);
    } else {
      enqueueSnackbar(' Client Failed to Load success!', {
        variant: 'error',
      });
      setTableData([]);
    }
  ...
  return (
    <>
      <Head>
        <title>Clients: List | Minimal UI</title>
      </Head>
