...
import { IClientCreateInput, ClientStatus } from 'src/@types/client';
import { useClient } from 'src/modules/client/hooks/useClient';
// ----------------------------------------------------------------------

interface FormValuesProps extends Omit<IClientCreateInput, 'avatarUrl'> {
  avatarUrl: CustomFile | string | null;
}

type Props = {
  isEdit?: boolean;
  currentClient?: IClientCreateInput;
};

export default function ClientNewEditForm({ isEdit = false, currentClient }: Props) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      contactMobile: currentClient?.contactMobile || '',
      ...
      landmark: currentClient?.landmark || '',
      city: currentClient?.city || '',
      pincode: currentClient?.pincode || '',
      state: currentClient?.state || 'Karnataka',
      status: currentClient?.status || ClientStatus.PENDING,
      country: currentClient?.country || 'India',

      pan: currentClient?.pan || '',
      gst: currentClient?.gst || '',
  ...
  const onSubmit = async (data: FormValuesProps) => {
    ...
    const client = {
      title: data.title,
      clientCommuncation: _communication,
      clientDocument: _documents,
      clientAddress: _address,
    };
    console.log('client: ', client);

    const _newItemCreated = await create(client);
        ...
        <Grid item xs={12} md={8}>
          ...
          <Card sx={{ p: 3, m: 2 }}>
            ...
            >
              ...
              <RHFTextField name="pan" label="client's PAD" />
