...
// @types
import { UserSearchParams, UserStatus, IUser } from '../../@types/user';
// _mock_
...
// sections
import { UserTableToolbar, UserTableRow } from '../../sections/user/list/index';
import { useUser } from 'src/modules/user/hooks/useUser';
import { useSnackbar } from 'src/components/snackbar';
import { buildUserWhereFilter } from 'src/modules/user/helpers/buildWhereFilter';
...
export default function userListPage() {
  ...
  const { getManyWithFilters, remove: deleteUser } = useUser();
  ...
  const [searchParam, setsearchParam] = useState<any | undefined>(UserSearchParams.TITLE)
  const [searchQ, setsearchQ] = useState("")
  const { enqueueSnackbar } = useSnackbar();


  async function getUsers(filter: any, reload?: any) {
  ...
  const handleDeleteRow = async (id: string) => {
    ...
    if (deletedUser?.data.success) {
      ...
      enqueueSnackbar(' This User Cant be Deleted');
    }
  ...
  const onSearchSubmit = () => {
    const _query = buildUserWhereFilter(searchParam, searchQ, filterStatus)
    getUsers({
      ..._query
    });
  };
  ...
  const handleEditRow = (id: string) => {
    push(PATH_USER.edit(paramCase(id.toString())));
  };
  const handleDetailRow = (id: string) => {
    push(PATH_USER.detail(paramCase(id.toString())));
  };
  ...
  _filterStatus = [_filterStatus, ...Object.keys(UserStatus)];


  let _searchParams: any[] = [];
  _searchParams = [_searchParams, ...Object.keys(UserSearchParams)];
  ...
  return (
    <>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading="Users"
          links={[{ name: 'All', href: PATH_USER.root }, { name: 'List' }]}
          action={
            <Button
              component={NextLink}
              href={PATH_USER.new}
