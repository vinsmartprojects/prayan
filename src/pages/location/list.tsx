...
// routes
import { PATH_DASHBOARD, PATH_LOCATION } from '../../routes/paths';
// @types
...
// sections
import { LocationTableToolbar, LocationTableRow } from '../../sections/location/list/index';
...
const TABLE_HEAD = [
  { id: 'title', label: 'Location Name', align: 'left' },
...
locationListPage.getLayout = (page: React.ReactElement) => <DashboardLayout>{page}</DashboardLayout>;
...
export default function locationListPage() {
  ...
  const { getMany, remove: deleteLocation } = useLocation();
  ...
  async function getVenodrs(reload?: any) {
    ...
    await _result;
    if (_result?.data) {
      enqueueSnackbar(reload ? 'Location reloaded Successfully' : ' Location Loaded successfully!');
      setTableData(_result?.data);
    } else {
      enqueueSnackbar(' Location Failed to Load success!', {
        variant: 'error',
      });
      setTableData([]);
    }
  ...
  const handleDeleteRow = async (id: string) => {
    ...
    if (deletedLocation?.data.success) {
  ...
  const handleEditRow = (id: string) => {
    push(PATH_LOCATION.edit(paramCase(id.toString())));
  };
  ...
  return (
    <>
      <Head>
        <title>Location: List | Minimal UI</title>
      </Head>

      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading="Locations"
          links={[{ name: 'All', href: PATH_LOCATION.root }, { name: 'List' }]}
          action={
            <Button
              component={NextLink}
              href={PATH_LOCATION.new}
              variant="contained"
