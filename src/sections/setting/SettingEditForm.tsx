...
import { ISetting, ISettingCreateInput, ISettingEdit, SettingStatus } from 'src/@types/setting';
import { useSetting } from 'src/modules/setting/hooks/useSetting';
import { useUploader } from 'src/modules/cdn/useUploader';
// ----------------------------------------------------------------------

interface FormValuesProps extends Omit<ISettingEdit, 'avatarUrl'> {
...
export default function SettingEditForm({ isEdit = false, setting }: Props) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      contactMobile: setting?.contactMobile || '',
      contactEmail: setting?.contactEmail || '',
      addressLine1: setting?.address?.addressLine1 || '',
      addressLine2: setting?.address?.addressLine2 || '',
      area: setting?.address?.area || '',
      landmark: setting?.address?.landmark || '',
      city: setting?.address?.city || '',
      pincode: setting?.address?.pincode || '0',
      state: setting?.address?.state || '',
      profileImage:
        (setting?.profileImage && { file: cdnPath(setting?.profileImage), isNew: false }) ||
        undefined,
      country: setting?.country || 'India',
      pan: setting?.pan || '',
      panDoc: (setting?.panDoc && { file: cdnPath(setting?.panDoc), isNew: false }) || undefined,
      gst: setting?.gst || '',
      gstDoc: (setting?.gstDoc && { file: cdnPath(setting?.gstDoc), isNew: false }) || undefined,
      estbId: setting?.estbId || '',
      estbtDoc:
        (setting?.estbtDoc && { file: cdnPath(setting?.estbtDoc), isNew: false }) ||
        undefined,
      cin: setting?.cin || '',
      cinDoc: (setting?.cinDoc && { file: cdnPath(setting?.cinDoc), isNew: false }) || undefined,
      isVerified: setting?.isVerified || false,
  ...
  const values = watch();
  useEffect(() => {
    console.log("setting ", setting?.isVerified)
  ...
  const onSubmit = async (data: FormValuesProps) => {
    ...
    if (data?.profileImage?.isNew === true) {
      ...
      if (_fileUploaded?.data?.filename) {
        _settingUpdateParams.profileImage = _fileUploaded?.data?.filename;
      }
    }
    if (data?.panDoc?.isNew === true) {
      ...
      await _fileUploaded;
      if (_fileUploaded?.data?.filename) {
        _settingUpdateParams.panDoc = _fileUploaded?.data?.filename;
      }
    }
    if (data?.gstDoc?.isNew === true) {
      ...
      if (_fileUploaded?.data?.filename) {
        _settingUpdateParams.gstDoc = _fileUploaded?.data?.filename;
      }
    }
    if (data?.estbtDoc?.isNew === true) {
      ...
      if (_fileUploaded?.data?.filename) {
        _settingUpdateParams.estbtDoc = _fileUploaded?.data?.filename;
      }
    }
    if (data?.cinDoc?.isNew === true) {
      ...
      if (_fileUploaded?.data?.filename) {
        _settingUpdateParams.cinDoc = _fileUploaded?.data?.filename;
      }
    }
    ...
    const _setting: any = {
      ...
      address: _address,
      ..._settingUpdateParams,
      ..._ids,
      ..._communication
    };
    try {
      const _updatedItem = await update(setting?.id, _setting);
      await _updatedItem;
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={8}>
          <Card sx={{ p: 3, m: 2 }}>
            ...
            >
              <RHFTextField name="title" label="Setting Title *" />
          ...
          <Card sx={{ p: 3, m: 2 }}>
            ...
            >
              ...
              >
                <RHFTextField name="gst" label="Setting's GST" />
                <RHFUploadAvatar
                  name="gstDoc"
              ...
              >
                <RHFTextField name="pan" label="Setting's PAN" />
                <RHFUploadAvatar
                  name="panDoc"
              ...
              >
                <RHFTextField name="cin" label="Setting's CIN" />
                <RHFUploadAvatar
                  name="cinDoc"
              ...
              >
                <RHFTextField name="estbId" label="Setting's Establishment  Doc" />
                <RHFUploadAvatar
                  name="estbtDoc"
        ...
        <Grid item xs={12} md={4}>
          ...
          <Card sx={{ mt: 3, pt: 2, pb: 2, px: 3 }}>
            ...
            >
              {' '}
              <Typography> Setting Credentials</Typography>
