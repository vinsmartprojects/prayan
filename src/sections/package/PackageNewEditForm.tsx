...
import { IPackageCreateInput, PackageStatus } from 'src/@types/package';
import { usePackage } from 'src/modules/package/hooks/usePackage';
// ----------------------------------------------------------------------

interface FormValuesProps extends Omit<IPackageCreateInput, 'avatarUrl'> {
  avatarUrl: CustomFile | string | null;
}

type Props = {
  isEdit?: boolean;
  currentPackage?: IPackageCreateInput;
};

export default function PackageNewEditForm({ isEdit = false, currentPackage }: Props) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      contactPerson: currentPackage?.contactPerson || '',
      contactMobile: currentPackage?.contactMobile || '',
      contactEmail: currentPackage?.contactEmail || '',

      addressLine1: currentPackage?.addressLine1 || '',
      addressLine2: currentPackage?.addressLine2 || '',
      area: currentPackage?.area || '',
      landmark: currentPackage?.landmark || '',
      city: currentPackage?.city || '',
      pincode: currentPackage?.pincode || '',
      state: currentPackage?.state || 'Karnataka',
      status: currentPackage?.status || PackageStatus.PENDING,
      country: currentPackage?.country || 'India',

      pan: currentPackage?.pan || '',
      gst: currentPackage?.gst || '',
      tin: currentPackage?.tin || '',
      cin: currentPackage?.cin || '',
      isVerified: currentPackage?.isVerified || false,
    }),
  ...
  const onSubmit = async (data: FormValuesProps) => {
    ...
    const package = {
      title: data.title,
      packageCommuncation: _communication,
      packageDocument: _documents,
      packageAddress: _address,
    };
    console.log('package: ', package);

    const _newItemCreated = await create(package);
        ...
        <Grid item xs={12} md={8}>
          ...
          <Card sx={{ p: 3, m: 2 }}>
            ...
            >
              ...
              <RHFTextField name="pan" label="package's PAD" />
              <RHFTextField name="tin" label="package's TIN" />
